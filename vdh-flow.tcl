...
puts "\n${CYAN}==========================================================================================${RESET}"
puts "${GREEN}                             WELCOME TO  V D H - F L O W${RESET}"
puts "${CYAN}------------------------------------------------------------------------------------------${RESET}"
puts "  A Fully Automated Open-Source RTL Synthesis & STA Flow"
puts "  Developed by:  ${GREEN}Karthiswaran R${RESET}"
puts "  Organization:  ${GREEN}VLSI Design Hub${RESET}"
puts "${CYAN}------------------------------------------------------------------------------------------${RESET}"
puts "  Description :  VDH-FLOW takes RTL Netlist and SDC constraints as inputs, performs"
puts "                 synthesis using Yosys, and generates pre-layout timing reports"
...
puts "\nvdh-flow Info: Listing key variables for debug:"
...
if {! [file exists $EarlyLibraryPath]} {
	puts "\nvdh-flow Error: Early library not found at $EarlyLibraryPath. Exiting..."
	exit
} else {
	puts "\nvdh-flow Info: Early cell library found at $EarlyLibraryPath"
}

if {! [file exists $LateLibraryPath]} {
	puts "\nvdh-flow Error: Late library not found at $LateLibraryPath. Exiting..."
	exit
} else {
	puts "\nvdh-flow Info: Late cell library found at $LateLibraryPath"
}

if {! [file isdirectory $OutputDirectory]} {
	puts "\nvdh-flow Info: Creating output directory $OutputDirectory"
	file mkdir $OutputDirectory
} else {
	puts "\nvdh-flow Info: Output directory found at $OutputDirectory"
}
...
if {! [file exists $ConstraintsFile]} {
	puts "\nvdh-flow Error: Constraints file not found at $ConstraintsFile. Exiting..."
	exit
} else {
	puts "\nvdh-flow Info: Constraints file found at $ConstraintsFile"
}

#====================================================================#
#                 Constraints File Parsing - SDC Creation
#====================================================================#
puts "\nvdh-flow Info: Dumping SDC constraints for $DesignName"
...
if {![file exists /tmp/1]} {
    puts "vdh-flow Warning: /tmp/1 not found — skipping port check for this input."
    set count 0
} else {
...
if {$count > 2} {
    ...
    puts "vdh-flow: Port [constraints get cell 0 $i] detected as BUS (count=$count)"
} else {
    set inp_ports [constraints get cell 0 $i]
    puts "vdh-flow: Port $inp_ports detected as SINGLE-BIT (count=$count)"
}
...
read_lib -early /home/vsduser/vdh-flow/osu018_stdcells.lib 

read_lib -late /home/vsduser/vdh-flow/osu018_stdcells.lib
